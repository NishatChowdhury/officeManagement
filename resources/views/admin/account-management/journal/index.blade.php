        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            {{ Form::open(['url'=>'admin/journals','method'=>'get','class'=>'form-inline']) }}
                    ...
                    <div class="card">
                        <div class="card-header d-flex">
                            ...
                            <div class="ml-auto media">
                                <a href="{{ url('admin/journals/create') }}" class="btn btn-success align-self-center"><i class="fas fa-plus-circle"></i> Create Journal</a>
                            </div>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <table id="example2" class="table table-bordered table-hover">
                                ...
                                @foreach($journals as $journal)
                                    <tr>
                                        <th><a class="btn btn-outline-primary btn-sm text-bold" href="{{ url('admin/journals',$journal->id) }}">{{ $journal->journal_no }}</a></th>
                                        <td>{{ $journal->date }}</td>
                                        <td>{{ $journal->items->sum('credit') }}</td>
                                        <td>{{ $journal->description }}</td>
                                        <td>
                                            {{ Form::open(['url'=>['admin/journals',$journal->id],'method'=>'delete','onsubmit'=>'return confirmDelete()']) }}
                                            <a href="{{ url('admin/journals',$journal->id) }}" class="btn btn-info btn-sm"><i class="fas fa-eye"></i></a>
